  ...
  switch (statement.type) {
    ...
    case 'ForStatement':
      if (statement.init) {
        ...
        else walkExpression(options, context, statement.init);
      }
      if (statement.test) walkExpression(options, context, statement.test);
      if (statement.update) walkExpression(options, context, statement.update);
      walkStatement(options, context, statement.body);
      break;
    case 'ForInStatement':
      if (statement.left.type === 'VariableDeclaration')
        walkStatement(options, context, statement.left);
      else walkExpression(options, context, statement.left);
    ...
    case 'FunctionDeclaration':
      ...
      if (statement.body.type === 'BlockStatement')
        walkStatement(options, context, statement.body);
      else walkExpression(options, context, statement.body);
...
function walkSwitchCases(options, context, switchCases) {
  switchCases.forEach(function(switchCase) {
    if (switchCase.test) walkExpression(options, context, switchCase.test);
...
function walkCatchClauses(options, context, catchClauses) {
  catchClauses.forEach(function(catchClause) {
    if (catchClause.guard) walkExpression(options, context, catchClause.guard);
  ...
  declarators.forEach(function(declarator) {
    switch (declarator.type) {
      case 'VariableDeclarator':
        ...
        if (declarator.init) walkExpression(options, context, declarator.init);
        break;
    }
  });
}
...
function walkExpression(options, context, expression) {
  switch (expression.type) {
    ...
    case 'FunctionExpression':
      ...
      else walkExpression(options, context, expression.body);
      context.overwrite.length = old;
      context.inTry = oldInTry;
      break;
    ...
    case 'CallExpression':
      ...
      if (
        ...
        var elements =
          expression.arguments[1].type == 'ArrayExpression'
            ? expression.arguments[1].elements
    ...
    case 'MemberExpression':
      if (
        ...
        { exports: 1, id: 1, loaded: 1 }.hasOwnProperty(
    ...
    case 'Identifier':
      if (
        ...
        context.overwrite.indexOf(expression.name) === -1 &&
