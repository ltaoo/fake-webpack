...
function webpack(
    ...
    options.parse.overwrites.process =
        options.parse.overwrites.process || '__webpack_process';
    options.parse.overwrites.module =
        options.parse.overwrites.module || '__webpack_module+(module)';
    // 当有该配置，会将 console 转为 __webpack_console
    // options.parse.overwrites.console = options.parse.overwrites.console || '__webpack_console';
    options.parse.overwrites.global =
        options.parse.overwrites.global || '__webpack_global';
    options.parse.overwrites.Buffer =
        options.parse.overwrites.Buffer || 'buffer+.Buffer';
    options.parse.overwrites['__dirname'] =
        options.parse.overwrites['__dirname'] || '__webpack_dirname';
    options.parse.overwrites['__filename'] =
        options.parse.overwrites['__filename'] || '__webpack_filename';
    ...
    buildDeps(context, moduleName, options, function(
        err: Error,
        depTree: DepTree,
    ) {
        if (err) {
        ...
        fs.writeFileSync(
            path.resolve(__dirname, '../dist/bundle.json'),
            _json,
            'utf-8',
        );
        ...
        for (let i = 0, l = chunkIds.length; i < l; i += 1) {
            ...
            buffer = [];
            const chunkTemplate = getTemplate.chunk(
                chunk,
                options,
                templateOptions,
            );
            ...
            if (chunk.realId === 0) {
                ...
                if (chunkIds.length > 1) {
                    ...
                    buffer.push(
                        JSON.stringify(
                            options.outputPostfix.replace(HASH_REGEXP, hash),
                        ),
                    );
                    buffer.push(',b:');
                ...
                buffer.push(
                    JSON.stringify(
                        options.publicPrefix.replace(HASH_REGEXP, hash),
                    ),
                );
                buffer.push(',\n');
            } else {
        ...
        function writeFiles() {
            ...
            fileWrites.forEach(function(writeAction) {
                const writeActionFileName = writeAction[0].replace(
                    HASH_REGEXP,
                    hash,
                );
                fileSizeMap[path.basename(writeActionFileName)] =
                    writeAction[1].length;
                fs.writeFile(
                    writeActionFileName,
                    writeAction[1],
                    'utf-8',
